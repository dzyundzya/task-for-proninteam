...
class EmailService:
    ...
    @classmethod
    def send_text_email(
        cls, subject: str, to_emails: list[str], message: str, 
    ) -> None:
...
class PaymentNotificationService:
    ...
    @classmethod
    def send_payment_paid_email(cls, payment: Payment) -> None:
        """Send email when payment is marked as paid."""
        user_name = payment.user.get_full_name() or payment.user.username
        
        message = f"""Hello, {user_name}!

        Your payment to collection "{payment.collect.title}" has been confirmed

        Details:
        • Collection: {payment.collect.title}
        • Amount: {payment.amount} rub.
        • Payment date: {
            payment.payment_day.strftime('%d.%m.%Y %H:%M') 
            if payment.payment_day else 'Not specified'
        }

        Thank you for your contribution! Your support is very important.
        ...
        EmailService.send_text_email(
            subject=f'Payment "{payment.collect.title}" confirmed',
            to_emails=[payment.user.email],
            message=message,
        )
