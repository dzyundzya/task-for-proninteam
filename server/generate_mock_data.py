...
class Command(BaseCommand):
    ...
    def _create_collects(
        self, fake: Faker, count: int, users: list[CustomUser]
    ) -> list[Collect]:
        collects = []
        ...
        for i in range(count):
            ...
            collect = Collect.objects.create(
                ...
                end_date=(
                    timezone.now()
                    + timezone.timedelta(days=fake.random_int(1, 365))
    ...
    def _create_payments(
        ...
        for i in range(count):
            ...
            Payment.objects.create(
                ...
                payment_day=(
                    timezone.now()
                    - timezone.timedelta(days=fake.random_int(0, 30))
                    if fake.boolean(chance_of_getting_true=70)
                    else None
                ),
            )
